...
export const HUMANIZE_PROMPT_VERSION = "khazix-lite-v2-imagery";
...
const humanizeIntensityRules: Record<HumanizeIntensity, string> = {
  light: "轻度：清理明显 AI 套话和生硬连接；开头与结尾都要有可感知的改写，并至少自然加入 1 处贴合主题的比喻。资讯和品牌表达仍需稳健。",
  medium: "中度：明显增强长短句节奏、自然转场和明确判断；重点重写开头与结尾，全文自然穿插 1-2 处贴合主题的比喻或意象，并在结尾回扣开头。专业内容、教程结构与平台语气必须保留。",
  strong: "较强：让读者明显感到卡兹克式的聊天感、画面感和余韵。重点重写开头与结尾，全文自然穿插 2-3 处比喻或意象，至少使用一次短句停顿，并让结尾回扣开头意象；仍不得使用粗口、攻击性表达或虚构第一人称经历。",
};
...
export function buildHumanizePrompt(
  ...
  return `你是资深中文编辑。请对下面已经完成 GEO 调优和平台适配的文章做“卡兹克 Lite 真人感润色”。这是一次受控的风格改写，不改变事实与信息结构，但必须让读者明显感到语言已经被重新打磨，不能只替换几个同义词。
...
- 去掉或自然改写“首先/其次/最后、综上所述、值得注意的是、不难发现、让我们来看看、接下来让我们、这意味着、本质上、换句话说”等模板化套话，但不要机械替换。
- 调整长短句节奏、重复句式和生硬转场，让表达像“有见识的普通人在认真聊一件事”。判断必须有原文依据。
- 开头是本次润色重点，不能沿用原稿前两段的原句或只做近义词替换。先从原稿已有的事实、矛盾、场景或核心问题中找一个具体切口，再用一个克制而有画面感的比喻、意象或反差把读者带进来。不得为了制造画面而虚构时间、地点、人物、动作或亲历。
- 结尾也是本次润色重点，不能用“综上”“总而言之”式总结。回到开头的意象或核心矛盾，让同一个意象在结尾获得新的含义，再用一句简短、有余韵但不空泛的话收住。
- 正文可在解释抽象概念、技术变化或行业关系时，偶尔加入贴合语义的比喻，例如门、桥、河流、坐标、齿轮、地基、航线等意象；比喻必须帮助理解，不能堆砌辞藻，不能连续多段都用，不能盖过数据和结论。
- 可以使用“像、仿佛、好比、如同、就像”等比喻连接词，也可以用不带连接词的隐喻。优先从文章自身已有概念生长出意象，不要套用“时代的浪潮、双刃剑、星辰大海、灯塔、破局者”等陈词滥调。
- 保留必要的小标题、列表、Q&A、表格和 GEO 结论，但这些结构周围的开场句、转场句、解释句和收束句应主动改写。结构保护不是逐句照抄。
- 原文没有第一人称经历时，严禁添加“我体验过、我们团队遇到过、我采访过、朋友告诉我”等经历、现场或对话。
- 不添加粗口、低俗表达、攻击性措辞、夸张感叹号、固定作者介绍、关注引导或 Skill 固定结尾。
...
export function buildHumanizeStyleRetryPrompt(prompt: string, issues: string[]) {
  return `${prompt}\n\n上一次润色的风格改造不够明显，未通过卡兹克 Lite 风格检查：${issues.join("；")}。请重新生成完整 JSON。重点重新设计开头和结尾，并按当前强度加入克制、贴合主题且帮助理解的比喻或意象。不能只做同义词替换，也不能为了文艺感虚构事实、经历或场景；数字、图片、链接、代码、表格、GEO 结构和平台调性仍须保留。`;
}
